...
class UpcommingBloc extends Bloc<UpcommingEvent, UpcommingState> {
  UpcommingBloc() : super(UpcommingInitial()) {
      ...
      (event, emit) async {
        try {
          ...
          Response response = await get(
            Uri.parse(
                "https://api.themoviedb.org/3/movie/upcoming?api_key=e2caf0429e0e758201deda52d3d196f8&page=$page"),
          );
    ...
    on<UpcommingEventSearchList>(
      (event, emit) {
        emit(UpcommingStateLoadedList(SearchA().search(listUpcommingMocies, event.text)));
      },
    );
  }
}
